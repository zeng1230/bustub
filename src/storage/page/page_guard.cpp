...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
  ...
  page_lock_.emplace(frame_->rwlatch_);
  is_valid_ = true;
}
...
ReadPageGuard::ReadPageGuard(ReadPageGuard &&that) noexcept 
    : page_id_(that.page_id_),
      ...
      page_lock_(std::move(that.page_lock_)) {
      that.is_valid_ = false;
      that.page_lock_.reset();
    }
...
void ReadPageGuard::Flush() { 
  BUSTUB_ASSERT(is_valid_, "tried to use a invalid wtire guard");
  ...
  std::vector<DiskRequest> request;
...
void ReadPageGuard::Drop() { 
  ...
  if(page_lock_.has_value() && page_lock_->owns_lock()) {
    page_lock_->unlock();
    page_lock_.reset();
  }
...
void WritePageGuard::Flush() { 
  BUSTUB_ASSERT(is_valid_, "tried to use a invalid write guard");
