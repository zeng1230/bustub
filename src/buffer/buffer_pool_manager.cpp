...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  ...
  {
    std::scoped_lock latch_(*bpm_latch_);
...
auto BufferPoolManager::GetAvailableFrame() -> std::optional<frame_id_t> {
  ...
  frame_id_t frame_id = victim_opt.value();
  auto frame = frames_[frame_id];

  BUSTUB_ASSERT(frame->page_id_.has_value(), "evicted frame must contain a page");
  page_id_t old_page_id = frame->page_id_.value(); 

  if(frame->is_dirty_) {
    DiskRequest req;
    req.is_write_ = true;
    req.page_id_ = old_page_id;
    req.data_ = frame->GetDataMut();
    auto fut = req.callback_.get_future();
    std::vector<DiskRequest> requests;
    requests.push_back(std::move(req));
